...
data.lhsk = as.data.frame(data.lhsk)
###############################################################################
...
cost=c(0.1,1,100) # Double selection
...
set.seed(seed);id.crit <- sample(nrow(data.lhsk))
seed <- seed+1

folds <- cut(seq(1,nrow(data.lhsk)),breaks=k,labels=FALSE)
new.form <- c()
...
for (w in 1: nrow(par.test)) {
  ...
  for(e in 1:k){
    ...
    TUNE <- data.lhsk[-holdoutIndexes, ]
    HOLDOUT <- data.lhsk[holdoutIndexes, ]
  ...
  repeat {
    ...
    set.seed(seed);three.col <- sample(1:ncol(data.lhsk[,-c(42)]),no.col)
    
    seed <- seed+1
    
    var.tmp <- names(data.lhsk[, three.col])
    ...
    if( (abs((rmse0-avg.rmse)/rmse0) < .0001 | iter >10000)&length(all.col)>0 ){
      ...
      for (t in 1:length(varnum)) {
        ...
        mod <- svm(form, data = data.lhsk, kernel = "polynomial", degree = par[1], epsilon= par[2], cost= par[3])
        
        ##### calculate trace(s),inner product
        polykernel <- polydot(degree = par[1], scale = 1/length(var.set), offset = 0)
        TRAINt <- as.data.frame(data.lhsk[,var.set])
        ...
        resid.tr <- resid.tr - predict(mod, data.lhsk)
        
      }
      
      test.error <- sqrt(mean((resid.tr)^2))
      s_sum <- sum(s_store)
      
      gcv_cal <- mean((resid.tr/(1-s_sum/nrow(data.lhsk)))**2)
      ...
      print(iter);print(rmse0);print(avg.rmse);print(test.error);print(gcv_cal);print(opt_col);print(varnum)
      break
    ...
    #print("calculate p-value of 3 var")
    tmptrace <- c()
    for(e in 1:k){
      res <- resid.tune[[e]]

#epsilon 11 seeds with wronf hyperparameters
...
seedList <- c(1, 50001, 100001, 150001, 200001, 250001, 25001, 75001, 125001, 175001, 225001)
seedTestError <- c()
seedSd <- c()
runiter <- c()
runtime <- c()
for (s in 1:length(seedList)){
  tic("each seed")
  seed <- seedList[s]
  all.testerr <- c()
  crit.col <- rep(list(c()), 5)
  ...
  #print(cri_varnum)
  #print(optpar)
  seedTestError <- c(seedTestError, avg.testerr)
  seedSd <- c(sd(all.testerr), seedSd)
  toc()
  
  ft_seed = toc()
  rt = ft_seed$toc - ft_seed$tic
  runtime <- c(runtime, rt)
}
print(seedTestError)
print(mean(seedTestError))
...
write.csv(my_dataframe, row.names = FALSE, file = "runtime2.csv")

# Optionally, you can specify 'row.names = FALSE' to exclude row names in the CSV file

a <- read.csv("runtime2.csv")
d <- c()
for (it in 1:11) {
  b <- a[(1+(it-1)*5*25):(it*5*25) , 1]
  print(mean(b))
  c <- mean(b)
  d <- c(c,d)
}
